...
def updateRplus(newRule, idn):
    ...
    file.write("\n" + newRule)
    file.close()
    file2 = open(Path("rPlus-PDF.txt"), "a", encoding="utf-8")
    file2.write("\n" + newRule)
    file2.close()
...
def filter_nodes(node):
    cleaned_nodes = []
    if node and node.isNodeType(LatexEnvironmentNode):
        if node.__getattribute__('environmentname') == 'document':
            for subnode in node.__getattribute__('nodelist'):
                cleaned_nodes.extend(filter_nodes(subnode))
        elif not node.__getattribute__('environmentname') in ['figure', 'table', 'tabular', 'tikzpicture', 'lstlisting',
                                                              'figure*', 'table*']:
    ...
    elif node and node.isNodeType(LatexMacroNode):
        if hasattr(node, 'nodelist'):
            for subnode in node.__getattribute__('nodelist'):
                cleaned_nodes.extend(filter_nodes(subnode))
        else:
            try:
                LatexNodes2Text().macro_node_to_text(node)
                cleaned_nodes.append(node)
            except IndexError:
                print('Text node not parsable. Skipped.')
    elif node:
        cleaned_nodes.append(node)
...
def stripTex(latex_string):
    [nodes, _, _] = LatexWalker(latex_string).get_latex_nodes()
    cleaned_nodes = []
    for node in nodes:
        cleaned_nodes.extend(filter_nodes(node))

    stripped = ''
    try:
        stripped = LatexNodes2Text().nodelist_to_text(cleaned_nodes)
    except IndexError:
        print('Document not parsable. Skipped.')
    stripped = stripped.replace("\n", " ")
    return stripped


def filterTex(path="../../arXiv-papers/tex/4.tar.gz"):
    try:
        tar = tarfile.open(path)
    except tarfile.ReadError:
        print('Could not read tar archive.')
        return ''
    ...
    for tex in tex_files:
        try:
            tex_file = tar.extractfile(tex).read().decode('UTF-8')
            text += stripTex(tex_file)
...
# loadsPaper and returns bodyText as List of sentences filtered: only numbers
def loadTarGz(path="../../arXiv-papers/tex/4.tar.gz"):
    text = filterTex(path)
    splitPos = matchPositions(r'\.\s?[A-Z]', text)
    splitText = splitAtPosition(splitPos, text)
    filteredText = filterSignificance(splitText)
    return filteredText


def loadPdf(path="../../arXiv-papers/pdf/4.pdf"):
