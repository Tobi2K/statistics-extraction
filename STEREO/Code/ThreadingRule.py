...
def collectMinusSentences(amount, directory="../../Cord-19/document_parses/pdf_json/", fileType='json',
                          minimized='none'):
    ...
    for t in range(cpuCount):
        ...
        threadList.append((lock, start, end, docList, ps, fileType, minimized))
...
def thread_getRminus(param):
    ...
    if minimized == 'min':
        rMinusList = pap.readRules("rMinus-minimized")
        print(len(rMinusList))
    else:
    ...
    for i in range(start, end):
        if fileType == 'json':
            ...
            docs = pap.loadPdf(ps[docList[i]])
        elif fileType == 'tex':
            docs = pap.loadTarGz(ps[docList[i]])
        for s in docs:
            # apply rules
