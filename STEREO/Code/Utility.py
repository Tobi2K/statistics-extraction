...
def applyRMinus(ruleList, sentence):
    ...
    g = open('match_count.txt', 'r').readlines()
    f = open('match_count.txt', 'w')
    matchList = []
    ruleid = 0
    for rule in ruleList:
        ...
        if ruleid < len(g):
            prev = int(g[ruleid])
        matches = prev + len(list(re.finditer(rule, sentence)))
        f.write(str(matches) + '\n')
    ...
    for s in doc:
        ...
        if match:
            ...
            if aw.checkRecord(entity):
                ...
                aw.gui.fixSubRules(entity, pap.readRules("subRule", ruleIndex), rPlusList[ruleIndex], ruleIndex)


def loadPaths(fileType='json'):
    ps = []
    if fileType == 'json':
        ps = glob.glob("../../Cord-19/document_parses/pdf_json/*.json")
    elif fileType == 'tex':
        ps = glob.glob("../../arXiv-papers/tex/*.tar.gz")
    elif fileType == 'pdf':
        ps = glob.glob("../../arXiv-papers/pdf/*.pdf")
    ps = list(map(lambda pa: pa.replace("\ ", "/"), ps))
    return ps


def getUncoveredSentences(amount, fileType='json'):
    ps = loadPaths(fileType)
    documentCounter = 501
    rPlusList = pap.readRules('rPlus')
    ...
    with open(Path('Evaluierung/Samples/extractedStatistics_uncoveredSamples.txt'), 'w+', encoding='utf-8') as file:
